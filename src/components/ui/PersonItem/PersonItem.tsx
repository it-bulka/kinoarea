...
export const PersonItem = memo(
  ({ img, name, known_for, character, rating, department, index, onClick }: PersonItemProps) => {
    ...
    const chips = known_for
      ? known_for.slice(0, 4).map(item => ({ id: item.id, name: (item as any).title ?? (item as any).name ?? '' }))
      : []
      ...
      <li className="actor-item group relative flex items-center gap-3 md:gap-4 lg:gap-7 py-4 md:py-5 border-b border-noir-border hover:bg-gold/[0.03] transition-colors duration-300 rounded-lg">
        ...
        <div className="flex-1 min-w-0 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-6">
          ...
          >
            ...
            {character && !known_for && (
              <span className="text-[10px] md:text-xs bg-noir-soft text-gold-light border border-noir-border/50 rounded-full px-2.5 py-[3px] font-inter font-medium truncate max-w-[160px]">
                {character}
              </span>
            )}
          </div>
